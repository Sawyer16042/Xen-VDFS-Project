...
//        total += v->avg_runstate.time[RUNSTATE_offline];
    ...
    do_div(tsc_khz, tsc_to_system_mul);
    if (tsc_shift < 0)
    	tsc_khz <<= -tsc_shift;
    else
        tsc_khz >>= tsc_shift;
    ...
    tsc_khz = 1000000ULL << 32;
    do_div(tsc_khz, _t.tsc_scale.mul_frac);
    if ((s8)_t.tsc_scale.shift < 0)
    ...
    else
    {
       ...
       /*if (d->domain_id == 0){
                BLARG++;
		BLARG = BLARG % 500;
		_u.tsc_to_system_mul = t->tsc_scale.mul_frac + (1000000 * BLARG);
                _u.tsc_shift         = (s8)t->tsc_scale.shift;
        }
        else{*/
                _u.tsc_to_system_mul = _t.tsc_scale.mul_frac;
                _u.tsc_shift         = (s8)_t.tsc_scale.shift;
        //}
 
    }
    //TRACE_6D(TRC_SCHED_VCPU_TIME, d->domain_id, v->vcpu_id, ratio, _u.tsc_to_system_mul, tsc_khz, _u.tsc_shift);
